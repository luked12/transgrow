...
class TransGrowWGANGPModel(pl.LightningModule):
    def __init__(self, data_shape, g_enc_net, g_enc_net_pretrained, g_dec_net, g_dec_net_pretrained, pe_max_len, pe_fusion_type, dim_img, dim_pe, dim_z, dim, depth, heads, dropout_trf, dropout_emb, prd_token_type, d_net, d_net_pretrained, lr, target_frame, losses_w, final_actvn, b1=0.0, b2=0.9, n_critic=5, lambda_gp=10):
        '''
        Parameters
        ----------
        ...
        # WGAN GP Parameter for optimization
        self.b1 = b1
        self.b2 = b2
    ...
    def compute_gradient_penalty(self, img_seq, img_seq_pred):
        """Calculates the gradient penalty loss for WGAN GP"""
        # # Random weight term for interpolation between real and fake samples
        alpha = torch.Tensor(np.random.random((img_seq.size(0), 1, 1, 1))).to(self.device)
        # # Get random interpolation between real and fake samples
        interpolates = (alpha * img_seq + ((1 - alpha) * img_seq_pred)).requires_grad_(True).to(self.device)
        # # Calculate critic
    ...
    def _forward(self, img_in, timedelta_in, timedelta_target, mask=None, z=None):
        x  = self.generator(img_in, timedelta_in, timedelta_target, mask=mask, z=z)
        return x
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        # # train generator
        if optimizer_idx == 0:
            # # compute classical WGAN g_loss
            g_loss = -torch.mean(self.discriminator(img_seq_pred))
        ...
        # # train discriminator
            ...
            # # Real images
            real_validity = self.discriminator(img_seq)
            # # Fake images
            fake_validity = self.discriminator(img_seq_pred)
            # # Gradient penalty
            gradient_penalty = self.compute_gradient_penalty(img_seq, img_seq_pred)
            # # Discriminator loss
            d_loss = -torch.mean(real_validity) + torch.mean(fake_validity) + self.lambda_gp * gradient_penalty
            self.log('loss_d', d_loss)
            
            return self.losses_w['weight_adv'] * d_loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # # this needs to be enabled in validation, just for computation of the penalty
        ...
        # # Discriminator loss
        d_loss = -torch.mean(real_validity) + torch.mean(fake_validity) + self.lambda_gp * gradient_penalty
        self.log('val_loss_d', d_loss, on_step=False, on_epoch=True)
        # # Reconstruction loss
        l1_loss = self.l1_criterion(img_pred, img_target)
        self.log('val_loss_l1', l1_loss, on_step=False, on_epoch=True)
        # # compute classical WGAN g_loss
        g_loss = -torch.mean(self.discriminator(img_seq_pred))
        self.log('val_loss_g', g_loss, on_step=False, on_epoch=True)
        # # compute msssim loss
        loss_msssim = self.loss_msssim(img_pred, img_target)
